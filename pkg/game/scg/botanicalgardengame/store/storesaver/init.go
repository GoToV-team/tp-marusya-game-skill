...
//
// BotanicalGardenGame-Store saver
//
// Command:
// scg
// DO NOT EDIT .
...
import (
	...
	"github.com/evrone/go-clean-template/pkg/game/scg/botanicalgardengame/store"
)

const checkKey = "BotanicalGardenGame-ad92c282-8040-4522-a54e-783f7bb5ac1c"
...
func saveScripts(st store.ScriptStore) error {
	...
	if err = st.SetText(consts.HelloText, `Привет! Добро пожаловать в игру "Ботанический сад".`); err != nil {
		return err
	}
	if err = st.SetText(consts.HelloTTS, `Привет! Добро пожаловать в игру "Ботанический сад".`); err != nil {
		return err
	}
	...
	if err = st.SetText(consts.StartRulesText, `Правила игры очень просты: в Вашем саду растет необычное дерево - денежное! Каждый день его листья приносят вам богатства. Но за таким чудным деревом нужен тщательный уход! Оно может завянуть, замерзнуть или быть атаковано злыми жуками!  У Вас есть семь дней, чтобы ухаживать за деревом и заработать как можно больше денег.  Перед каждым игровым днём Вы узнаете прогноз погоды и цены на товары в магазине и принимаете решение, какие товары необходимо купить, чтобы дерево чувствовало себя комфортно и росло. По истечении семи игровых дней Вы увидите, сколько денег Вы заработали.`); err != nil {
		return err
	}
	if err = st.SetText(consts.StartRulesTTS, `Правила игры очень просты: в Вашем саду растет необычное дерево - денежное! Каждый день его листья приносят вам богатства. Но за таким чудным деревом нужен тщательный уход! Оно может завянуть, замерзнуть или быть атаковано злыми жуками! У Вас есть семь дней, чтобы ухаживать за деревом и заработать как можно больше денег. Перед каждым игровым днём Вы узнаете прогноз погоды и цены на товары в магазине и принимаете решение, какие товары необходимо купить, чтобы дерево чувствовало себя комфортно и росло. По истечении семи игровых дней Вы увидите, сколько денег Вы заработали.`); err != nil {
		return err
	}
...
func clearOldStores(st store.ScriptStore) error {
	...
	keys, err := st.GetAllTextKeyForScript("BotanicalGardenGame")
	if err != nil {
		return err
	}
